...
public class Player : MonoBehaviour
{
        ...
        if (RACE_MANAGER.RACE_STARTED && !item_manager.isBullet)
        {
            ...
            if(beforeStartAccelTime > 1 && beforeStartAccelTime < 2)
            {
                ...
                for(int i = 0; i < 60; i++)
                {
                    ...
                    {
                        currentspeed = Mathf.SmoothStep(currentspeed, boost_speed, 1.75f * Time.deltaTime);
                    }
                }
            }
            ...
            if(collideCooldown <= 0)
            {
                ...
                if (dir < 0)
                {
                    ...
                    if (item_manager.StarPowerUp)
                    {
                        ...
                        StartCoroutine(collision.gameObject.GetComponent<OpponentItemManager>().hitByShell());
                    }
    ...
    void Move()
    {
        ...
        if (JUMP_PANEL)
        {
            downForce = Mathf.Lerp(downForce, 150000, 3 * Time.deltaTime);
    ...
    void lookatOpponent()
    {
        if (!invincible && !Driver.GetCurrentAnimatorStateInfo(0).IsName("New State"))
        {
            //                        kart.rotation = Quaternion.SlerpUnclamped(kart.rotation, Quaternion.Euler(kart.eulerAngles.x,kart.eulerAngles.y, 40), gliderSlerpRate * Time.deltaTime);

            Quaternion rot = Quaternion.LookRotation(GameObject.FindGameObjectWithTag("Opponent").transform.position - bone.position);
            rot = Quaternion.Euler(new Vector3(rot.eulerAngles.x, 0, 0));
