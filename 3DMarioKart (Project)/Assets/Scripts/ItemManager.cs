...
public class ItemManager : MonoBehaviour
{
    ...
    IEnumerator spawnRedShell(Transform position, int direction)
    {
        ...
        else if(direction == -1)
        {
            ...
            clone.GetComponent<GreenShell>().myVelocity = -transform.GetChild(0).forward.normalized * 5500 * Time.deltaTime;
    ...
    IEnumerator useBobomb(int direction)
    {
        if(direction == -1)
        {
            ...
            clone.GetComponent<Bobomb>().bomb_thrown(transform.InverseTransformDirection(GetComponent<Rigidbody>().velocity).z * 500);
    ...
    IEnumerator spawnBanana(int direction)
    {
        if(direction == 1)//forward
        {
            ...
            GameObject clone = Instantiate(banana, BananaSpawnPos.position, BananaSpawnPos.rotation);
        ...
        {
            yield return new WaitForSeconds(0.25f);
            GameObject clone = Instantiate(banana, backshellPos.position, BananaSpawnPos.rotation);
            for (int i = 0; i < 75; i++)
            {
                if (!StarPowerUp)
                    player_script.current_face_material = player_script.faces[1]; //make sure it is not changed, by repeating in for loop
