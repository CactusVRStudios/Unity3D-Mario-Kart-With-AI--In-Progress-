...
public class OpponentItemManager : MonoBehaviour
{
    ...
    void usingItem()
    {
        ...
        else if(current_item == "Banana")
        {
            ...
            {
                if(ai_script.nodes[ai_script.current_node].tag != "DriftLeft" && ai_script.nodes[ai_script.current_node].tag != "DriftRight")
                {
                    StartCoroutine(spawnBanana(1));
                    already_has_item = false;
                    ai_script.DriverAnim.SetBool("hasItem", false);
        ...
        else if(current_item == "TripleBananas")
        {
            if(tripleItemCount > 0 && tripleitemActionTime > 4)
            {
                ...
                {
                    if (ai_script.nodes[ai_script.current_node].tag != "DriftLeft" && ai_script.nodes[ai_script.current_node].tag != "DriftRight") //if driver is not drifting (not making a sharp turn), throw forward
                    {
                        StartCoroutine(spawnBanana(1));
                        //forward animation
                        tripleitemActionTime = 0;
